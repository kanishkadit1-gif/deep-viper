...
- **Shape classification & shape-aware targeting** — every detected object also
  gets a `shape` (circle / square / rectangle / triangle) from its contour, using
  circularity **and** extent (area ÷ oriented-bbox area: a square fills ~0.95 of its
  box, a circle ~0.785, a triangle ~0.5 — circularity alone can't separate a square
  from a circle since both ≈0.785). `vision.find_target(image, goal)` then selects
  the object matching the goal's **color and shape**, so "move the red **circle**"
  picks the ring, not the largest red object. Used by `pivot.generator.resolve_origin`.
