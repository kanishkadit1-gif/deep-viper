...
methods. Representative results on the current synthetic dataset (17 scenes,
offline heuristic selection — `USE_VLM=False`):

| Method | Task success | Collision rate |
|---|---|---|
| `random` | 0.12 | 0.47 |
| `pivot_only` | 0.00 | 0.47 |
| `vlmpc_only` | 0.24 | 0.59 |
| `full` | 0.65 | 0.47 |
| `full_with_routing` | 0.65 | **0.42** |

Reading: VLMPC validation (`full`) far outperforms trusting the proposal
(`pivot_only`) or random/PIVOT-free baselines; adding the route-around candidate
(`full_with_routing`) does not hurt success and **lowers the collision rate** (the
routed candidate is collision-free by construction). Numbers regenerate on each
`--benchmark`; a `vlm_sample` block in `metrics_summary.json` records whether a
real Claude call was exercised (it falls back to heuristic without an API key).
...
### 18.1 Generic object detection (`vision.py`)
...
- **`orchestrator/interactive.py`** + CLI `--detect-corridor` (list obstacles on
  ...
  moving anything, even when moving is permitted).
...
- `metrics_summary.json` — aggregate metrics for the full method over the dataset:
  ...
  goal.)
- `baseline_comparison.png` — task-success comparison of `full` vs baselines
  `pivot_only` (no VLMPC validation), `vlmpc_only` (random proposals + VLMPC), and
  `random`. The full method outperforms all three.
