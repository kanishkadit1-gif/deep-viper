...
| A* pather (Step 9 optional) | ✅ Complete | Swappable via `PATHER` config + sidebar |
| Bug 1: obstacle detection wired | ✅ Fixed | `localization/obstacles.py` A+B pipeline live |
| Bug 2a: Gemini model name | ✅ Fixed | `gemini-3.1-flash-lite` (was spaces/capitals) |
| A+B obstacle pipeline | ✅ Implemented | Gemini labels → OWL-ViT tight boxes → NMS |
