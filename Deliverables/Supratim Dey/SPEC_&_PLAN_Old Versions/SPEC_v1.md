...
No tool does another tool's job. In particular, the LLM never returns pixel coordinates.
...
- Discard hallucinated obstacle boxes spanning >80% of image width/height.
- Obstacles + object localization detected ONCE per run, reused across hops (no per-hop VLM/detector calls).

### Obstacle pathing — OPEN DECISION (greedy detour vs A*)
The committed-detour heuristic works for isolated obstacles but can get trapped when obstacles are CLUSTERED between mover and goal (it is a local greedy dodge, not a global planner).
- Option 1 (current): committed greedy detour + graceful stop.
- Option 2 (recommended): A* grid pathfinder — overlay a grid, mark obstacle cells blocked, A* from start to goal -> guaranteed path or a clean "no path". Eliminates the detour-thrashing class. The pather must be swappable via config (PATHER).
...
│   ├── detector.py                 # OWL-ViT / Grounding DINO load-once + locate(description)->box
...
GEMINI_MODEL         = "gemini-3.1-flash-lite"   # 500 RPD free vs 20 on 2.5-flash
USE_LLM_PARSER       = True         # parse messy prompts -> {source, target}

# localization
USE_DETECTOR         = True
DETECTOR_MODEL_ID    = "google/owlvit-base-patch32"   # or Grounding DINO
...
# outputs
GIF_FPS              = 6
SAVE_TRAIL_STILL     = True         # write trail.png (the trail-line still image)
