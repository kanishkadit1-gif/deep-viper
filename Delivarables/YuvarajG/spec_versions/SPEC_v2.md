...
- **Goal representation**: natural-language direction strings, mapped to pixel targets by the rules in §16.5. No goal reference image is required.
- **Dataset**: pre-extracted PNGs from Language Table `env.reset()` in `data/images/`. Extraction (§7) is a one-time offline step.
