...
Secrets and a few toggles are read from a gitignored **`.env`** file (loaded by a
tiny built-in parser in `config.py` — no `python-dotenv` dependency; real
environment variables still take precedence). `.env.example` is the template:
- `ANTHROPIC_API_KEY` — enables the VLM path when present (see §16.3, §17.1).
- `USE_VLM` (`true`/`false`/`1`/`0`) — overrides the `USE_VLM` default above.
- `PX_PER_CM` — explicit pixels-per-cm calibration (see §18.2); the `--px-per-cm`
...
- **Open-vocabulary, instruction-driven targeting** — `vision.find_target(image,
  goal)` resolves *which* object the instruction means. Primary path (Grounded-SAM):
  `vision.extract_object_phrase` pulls the object phrase from the goal (e.g. "move
  the red circle to …" → "red circle"; "move the wheel …" → "wheel"), GroundingDINO
  grounds that phrase directly, and the best box maps to the nearest detected object.
  This means the target can be **any phrase the model can ground** — not a fixed
  vocabulary (verified: "wheel" correctly selects the ring). Fallback path
  (classical / no model): each object also gets a `shape` (circle / square /
  rectangle / triangle) via circularity **and** extent (a square fills ~0.95 of its
  bbox, a circle ~0.785, a triangle ~0.5 — circularity alone can't separate square
  from circle, both ≈0.785), and the target is matched by **color + shape**. Used by
  `pivot.generator.resolve_origin`.
...
- **Explicit calibration (`--px-per-cm` / `PX_PER_CM`).** The card-size estimate is
  only reliable when the scene actually contains a card-sized reference; with larger
  objects (a laptop, a notebook) it can over-estimate the scale by ~10× and demand
  an absurd clearance. So `autoscale()` first honors an explicit override
  (`config.PX_PER_CM_OVERRIDE`, set by the `--px-per-cm` flag or the `PX_PER_CM`
  env/`.env` value) and skips the heuristic entirely; when it does fall back to the
  heuristic it **prints a warning** advising `--px-per-cm`. Explicit calibration is
  the recommended path for any real photo.
...
its tool set (`orchestrator/tools.py`) now includes **`route_around`** (A* over free
space, §18.3) alongside `plan_direct_push`, `plan_wide_push`, and `decompose_plan`,
and the system prompt places it before `decompose_plan` (route around before moving
anything). This closes a prior gap where the offline controller could route around
obstacles but the live agent could not, so it escalated on scenes the offline path
solved.
...
`PX_PER_CM_OVERRIDE` (explicit calibration, §18.2).
...
> **All run modes emit the full set.** The five core outputs **and** the per-run
> analysis artifacts below are produced by the plain pipeline *and* by the
> agent / interactive / chat paths (used for real-image runs) — they all route
> through `orchestrator.agent.write_outputs` (`analysis.write_agent_analysis`). For
> agent/interactive/chat runs, `log.json` carries the **AgentPlan** decision trace
> (also written to `agent_trace.json`) rather than the plain-pipeline `LogEntry`
> schema, and `vlm_reasoning.txt` holds the agent's reasoning trace.
...
## 20. Instruction Understanding (`instruction.py`)

Free-form instructions are parsed into a structured `Instruction` rather than a
fixed keyword grammar:

- `target` — the object phrase to move (open-vocabulary, via `find_target`).
- `dest_kind` — `corner` | `direction` | `on` | `between` | `near` | `point`.
- `direction` / `refs` — destination details; `refs` are *referenced object
  phrases* for relational destinations.
- `may_move_others` / `movable` — whether the text permits moving other objects.

**Two backends:**
- **LLM (Claude)** when `USE_VLM` + `ANTHROPIC_API_KEY` — one call returns the JSON.
- **Deterministic fallback** (always available, runs offline) — keyword + relational
  parsing. This is what executes when the LLM is unavailable (e.g. no API credits).

`instruction.resolve_destination(image, goal, origin, shape)` turns the parsed
destination into a pixel target, resolving **relational** destinations by grounding
the referenced objects in the image:
- `between A and B` → midpoint of the two grounded objects' centres (verified:
  "between the green square and the yellow square" → their midpoint).
- `near X` / `on X` → the grounded object's centre (note: `on`/stacking is not
  physically modelled in the 2D push surrogate — the planner treats X as an
  obstacle, so a true stack would escalate).
- `corner` / `direction` → the geometric mapping (`geometry.goal_to_target`).

`orchestrator.planner.perceive` now resolves the goal pixel through this layer, so
**every** entry point (run / agent / chat / interactive) understands relational
destinations. Move-permission can be parsed from text via `main.py --allow-move
auto`. The fixed shape/color/direction vocabulary remains only as the offline
fallback; the object **and** the destination are now instruction-driven.

---

## 21. Demo UI (`app.py`)

A self-contained local web app for demos — **no external dependencies** (Python
stdlib `http.server` + inline HTML/JS, no CDN), so it runs offline behind the
corporate proxy.
...
- **Upload** a 2D top-down image → it detects objects and shows a labelled scene
  preview (`/api/upload`).
- **Chat**: type an instruction; the agent runs the full pipeline and **replies with
  its reasoning** (status, explanation, decision trace, and any suggestions), while
  the output panel displays the artifacts (final plan, scene analysis, candidates,
  trajectory GIF, comparison grid, cost chart).
- **Multi-turn**: reply again to refine — e.g. if the agent escalates with
  suggestions, say "you can move the blocking objects" and it re-plans (permission is
  parsed from natural language via `instruction.parse` → interactive planning).

The image is sent as base64 JSON (no multipart), decoded server-side. Pipeline runs
are serialized under a lock. Uses `config.DETECTOR` (Grounded-SAM or classical); the
first run loads the model (~15 s), then detections are cached.

---

## 22. Configuration, Live-Agent Routing & Explicit Calibration (v17)

This version hardens deployment ergonomics and closes a live-agent capability gap;
all three are additive and keep the offline default path unchanged.

### 22.1 `.env` configuration loader (`config.py`)

`config.py` now loads a gitignored **`.env`** file via a tiny built-in parser (no
`python-dotenv` dependency, so the offline path stays dependency-free). Real
environment variables always win over file values. Keys: `ANTHROPIC_API_KEY`,
`USE_VLM` (`true`/`false`/`1`/`0`), and `PX_PER_CM`. `.env.example` is the committed
template; `.env` and generated `outputs/` are gitignored.

### 22.2 `route_around` as a live-Claude tool (`orchestrator/tools.py`, `prompts.py`)

The live tool-use orchestrator (`USE_VLM=True` + key) previously had only
`plan_direct_push`, `plan_wide_push`, `decompose_plan`, and `finalize`, so on scenes
where obstacles flank (not block) the route it escalated — even though the offline
controller and the interactive planner could already route around them via A*
(§18.3). It now also exposes **`route_around`** (one collision-free curved A* path to
the goal, moving nothing), and the decision policy places it at step 4 — after
direct/wide pushes, **before** `decompose_plan` (prefer disturbing nothing). The
`finalize` `strategy` enum gains `"route"`. Verified: on a real photo (green ring →
bottom-left past two flanking objects) the live agent now reaches `status=success`
via `route_around`, matching the offline planner.

### 22.3 Explicit `--px-per-cm` calibration (`vision.autoscale`, `main.py`, `config.py`)

The card-size autoscale heuristic (§18.2) assumes the median detected object is an
8.56 cm card; with larger objects it over-estimates scale ~10× and demands an
infeasible clearance. A new **`--px-per-cm`** CLI flag (and persistent `PX_PER_CM`
in `.env`) sets `config.PX_PER_CM_OVERRIDE`, which `autoscale()` honors before the
heuristic (short-circuiting detection). When it does fall back to the heuristic it
now prints a warning recommending explicit calibration. Resolution order:
`--px-per-cm` flag → `PX_PER_CM` env/`.env` → card-size heuristic.
